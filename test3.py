...
def calculate_centroid_and_draw(r, image_to_draw_on, target_class=1):
    """
    Tính trọng tâm (x_center, y_center) chỉ cho class target_class
    và vẽ lên ảnh image_to_draw_on (ảnh đã có box + mask).
    """
    centroids = []
    ...
    for i, mask_tensor in enumerate(masks_data):
        ...
        if M["m00"] != 0:
            ...
            cv2.circle(image_to_draw_on, (cx, cy), 4, (0, 0, 255), -1)  
        else:
            centroids.append((None, None))
...
output_image_dir = 'runs/segment/predict_with_centroids_5'
...
with open(OUTPUT_CSV_FILE, mode='w', newline='') as file:
    ...
    for r in results:
        ...
        im_result_with_centroids = r.plot()

        # chỉ tính trọng tâm cho class 1
        center_points, im_result_with_centroids = calculate_centroid_and_draw(r, im_result_with_centroids, target_class=1)
        ...
        if r.boxes is not None and len(r.boxes) > 0:
            ...
            for i in range(len(boxes_xyxy)):
                ...
                cx, cy = center_points[i] if i < len(center_points) else ("N/A", "N/A")
                writer.writerow([
                    file_name, i + 1, class_name, f"{score:.2f}",
                    x1, y1, x2, y2, cx, cy
                ])
        else:
